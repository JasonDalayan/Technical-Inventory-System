...
<body>
	...
	$Q = mysqli_query($con, "SELECT	* FROM login ORDER BY username ASC");
	$ctr = 0;

	$header = array('#','username','email','Action');

	echo "<div class='container'>";
		echo "<div class='tb_container'>";
			echo "<h2>View admin users</h2>";
			...
			echo "<tbody>";
				...
				echo "<td>"."<a href='editadminuser.php?id={$list['id']}'><input type='button' class='editfix' value='Edit'></a>"."<input type='submit' name='delete' class='editdispose' value='delete'>"."</td>";
			echo "</tr>";
			}
			echo "</tbody>";
			echo "</table><br>";
			echo "<form action='export' method='post'>";
			echo "<input type='submit' name='addadmin' class='edititems' value='Add admin'>";
			echo "</form>";
		echo "</div>";
	echo "</div>";
	echo "</form>";
	...
	if (isset($_POST['delete'])) {
		$arch = mysqli_query($con, "INSERT INTO admin_archived (tech_number,username,password,mail,usertype) values ('".$_SESSION['tech_number']."', '".$_SESSION['username']."','".$_SESSION['password']."', '".$_SESSION['mail']."', '".$_SESSION['usertype']."')");

		$del = mysqli_query($con, "DELETE FROM login WHERE username = '".$_SESSION['username']."' ");

		echo "<script>alert('the useradmin has been deleted')</script>";
